...
function cmdVideo2Wav(callback) {
    ...
    inquirer.prompt({
        ...
        basePath: path.resolve(__dirname,"../"),
            ...
            const output = path.resolve(outDir, `${baseName}_%04d.wav`)
            ...
            command
                ...
                .on('end', () => {
                    const fileList = fs.readdirSync(outDir)
                                            .filter((file)=> new RegExp(`${baseName}_[0-9]{4}\.wav`).test(file))
                    const fileInfo = {}
                    Promise.all(fileList.map((e, i, ary)=>{
                        const oldName = path.resolve(outDir, e)
                        return new Promise((resolve, reject)=>{
                                wavFileInfo.infoByFilename(oldName, (err, info) =>{
                                if(err){
                                    fileInfo[oldName] = 0
                                    resolve()
                                    return
                                }
                                fileInfo[oldName] = info.duration
                                resolve();
                            })
                        });
                    })).then(()=>{
                        ...
                        keys.sort();
                        var duration = Number(inputs.starttime);
                        for (let k of keys) {
                            const fixTime = Math.round(duration + fileInfo[k])
                            const newName = path.resolve(outDir, k.replace(/[0-9]{4}\.wav/,
                                duration < 3000 ?
                                `${moment.duration(duration, "s").format("mmːss",{ trim: false })}-${moment.duration(fixTime, "s").format("mmːss",{ trim: false })}.wav`:
                                `${moment.duration(duration, "s").format("hhːmmːss",{ trim: false })}-${moment.duration(fixTime, "s").format("hhːmmːss",{ trim: false })}.wav`))
                            fs.renameSync(k,newName)
                            duration = fixTime
                        }
                        logUpdate('変換中... 100%')
                        logUpdate.done()
